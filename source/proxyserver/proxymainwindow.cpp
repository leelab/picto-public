...
/*! \brief Creates a label for displaying the Session DB Name corresponding to the file
 *  the sqlite file the session currently active stores its experiment data into.
 */
void ProxyMainWindow::createSessionDBNameLabels()
{
	sessionDBFileLabel_ = new QLabel(tr("Session DB:"));
	sessionDBFileName_ = new QLabel();
	sessionDBFileName_->setTextInteractionFlags(Qt::TextSelectableByMouse | Qt::TextSelectableByKeyboard);
}
...
void ProxyMainWindow::createLayout()
{
	...
	HLayout = new QHBoxLayout();
	HLayout->addWidget(sessionDBFileLabel_);
	HLayout->addWidget(sessionDBFileName_);
	layout_->addLayout(HLayout);
...
{
	QString sessionDBName_ = QString();
	if (statusManager_)
	{
		sessionDBName_ = statusManager_.staticCast<ProxyStatusManager>()->getSessionDBName();
	}
	...
	case WaitForConnect:
		...
		sessionDBFileName_->setText("");
		sessionDBFileName_->update();
	...
	case WaitForDevice:
		...
		sessionDBFileName_->setText(sessionDBName_);
		sessionDBFileName_->update();
		pluginCombo_->setEnabled(true);
...
/*! \brief Stores application settings to disk
 *
 *	Settings for the workstation app are stored between sessions.
 *	The QSettings object does this for us in a platform independent
 *	manner.  This uses the registry in Windows, XML preference files
