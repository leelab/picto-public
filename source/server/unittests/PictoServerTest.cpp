...
void PictoServerTest::directorTest_data()
{
	QTest::addColumn<ACTOR_PARAM>("simActorDescriptor");
	QTest::addColumn<ACTION_PARAM>("simActionDescriptor");

	// Make the server discoverable and supply it with a list of ordered commands
	...
	desc->AddMsg(QSharedPointer<SimMsgDesc>(new Server::InterfaceDirectorDesc::StartMsgDesc(50)));
	desc->AddMsg(QSharedPointer<SimMsgDesc>(new Server::InterfaceDirectorDesc::StopMsgDesc(250)));
	...
	QTest::newRow("3") <<  ACTORPARAM(DIRECTOR,"Director0") <<  ACTIONPARAM( desc );
	// Make sure all the previous commands have time to occur.
	QTest::newRow("4") <<  ACTORPARAM(TESTBENCH) <<  ACTIONPARAM( new TestBench::SleepDesc("Wait for everything to happen",500));
	QTest::newRow("CLOSE") <<  ACTORPARAM(TESTBENCH) <<  ACTIONPARAM( new TestBench::CloseDevicesDesc("Close Devices") );


}
void PictoServerTest::directorTest()
{
	runExperiment();
}
