...
#include "../controlelements/TargetFactory.h"
	...
	addingTransition_(false)
{
	...
	controlTargetFactory_->addAssetType("TargetFactory",
		QSharedPointer<AssetFactory>(new AssetFactory(0, -1, AssetFactory::NewAssetFnPtr(TargetFactory::Create))));
}
...
bool MachineContainer::addTransition(QSharedPointer<Transition> transition)
{
	...
	//Set the pointers to source, sourceResult, and Destination into the transition so that it will be updated
	//  if their names change.
	//(Note that a name change would mess up our transitions_ list since it is indexed by name, but that only
	//  matters in an experiment.  In a real experiment the name won't ever change, so it is irrelevant.
	//We should never need to reset the asset pointers in a transition.  If the transition already has these
	//  pointers, we're probably just resetting the same pointers to it, and if we must be in a copy, paste
	//  situation where two assets temporarily have the same name and asset id.  In this case, we don't want
	//  the transition to move from one asset source/result/dest to another so we should skip the setting as well.
...
/*! \brief Adds the input ResultContainer element to this MachineContainer's elements_ lookup table, with the element's
*	name as the table key.
*/
...
void MachineContainer::postDeserialize()
{
	...
	//If we add a new child after deserialization (ie. In the State Machine Editor), we'll need to update the element and
	//  transition lists.
	//To do this, we connect childAddedAfterDeserialize() and childWasAdded() which calls updateListsFromChildren().
	//  We do it here (and not earlier) because making this connection during deserialization will create an exponential
	//  algorithm that will slow everything down.
	connect(this, SIGNAL(childAddedAfterDeserialize(QSharedPointer<Asset>)), this, SLOT(childWasAdded(QSharedPointer<Asset>)));
}

/*! \brief Extends OutputElementContainer::validateObject() to add checking for valid Transition definitions, checking that
*	all child element's results are connected to a transition, and checking to make sure no two elements have the same name.
*/
bool MachineContainer::validateObject(QSharedPointer<QXmlStreamReader> xmlStreamReader)
{
	...
	if (names.size()>0)
	{
		for (QStringList::iterator it = names.begin(); it != (names.end() - 1); it++)
		{
			...
			{
				QString errMsg =
					QString("MachineContainer: Two components of a MachineContainer can not have the same name: \"%1\".")
					.arg(*it);
				addError(errMsg);
				return false;
			}
...
/*! \brief Returns the source, sourceResult, and destination ResultContainer objects that are rerferenced in the input
*	transition.
*	\details This function looks up these Asset's according to the names stored in the Transition.  If there is some
*	problem in finding the Assets due to invalid names or ambiguity, the function returns false.  Otherwise, the function
*	returns true and the output data is avaiable in the reference parameters.
*/
bool MachineContainer::getTransitionAssets(QSharedPointer<Transition> transition, QSharedPointer<ResultContainer>& source,
	QSharedPointer<ResultContainer>& sourceResult, QSharedPointer<ResultContainer>& destination)
{
...
/*! \brief Called when a child is added to this MachineContainer.  Calls updateListsFromChildren() to update all of the
*	various internal lists accordingly.
*/
...
/*! \brief Repopulates all internal lists (element and transition lists) according to the current child elements and their
*	types.
*/
