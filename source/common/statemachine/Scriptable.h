 ...
 *	Any script in a Picto Design can access Scriptable elements that are the direct children of any of their ancestor elements.
 *	In javascript Scriptable elements are accessed just like any other javascript object. ie.
 *	\code
		scriptableElement.value = "new value";
		scriptableElement.someFunction(param1,param2);
	\endcode
 *	\details Adding script properties (ie. "value" in the example above) and script functions (ie. someFunction() in the example above) is straightforward.
 *	Every Q_PROPERTY in a class that inherits from Scriptable will appear as a script property in the script environment.  Every public Slot in a class that
 *	inherits from Scriptable will appear as a script function that can take inputs and return values.  C++ types are generally mapped in the most logical way
 *	to javascript types; however, it is easiest to think of the javascript types as more or less the same as a QVariant.  
 *
...
public:
	...
	/*! brief Returns true if a widget for adjusting this element's runtime editable Properties' initValues will show up in TestViewer and RemoteViewer UIs.*/
	 ...
	 *	set the order in which UIEnabled elements appear in widget boxes.  This would store this elements position in the widget box.
