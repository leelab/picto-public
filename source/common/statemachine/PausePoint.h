 ...
 *	\details Picto Tasks are tightly timed such that it really does not make sense to allow the operator to pause execution
 *	at any time.  If an Experiment were paused while a Fixation Point appeared on screen for example, we would effectively
 *	be giving the subject extra time to fixate on that point.  Ideally, we should be able to pause execution without
 *	affecting the integrity of Session data afterward, and that is the purpose of this element.  PausePoints are the place
 *	in control flow where execution actually pauses after the pause button is pressed.  In a Task with a trial structure,
 *	for example, the PausePoint should be placed just before or after the State handling the inter-trial interval so that
 *	any time pause is pressed the ITI will just be slightly longer and actual experimental data will not be affected.
 *
 *	Sometimes it is useful to be able to set up an automatic pause point as well.  For example, when we need data in blocks
 *	of a specific number N of trials, we can set up a PausePoint to automatically Pause execution as soon as the Trial
 *	counter reaches N.  This way the operator doesn't have to keep an eagle eye on the trial number, but can focus on
 *	other tasks with the confidence that execution will stop when the time is right.  For this purpose, PausePoint objects
 *	contain a "ForcePause" Property which, when set 'true' causes execution to Pause when the PausePoint is reached
 *	regardless of whether the pause button was pressed.
 *
 *	Regardless of whether pausing occured due to the pause button being pressed or "ForcePause" being true, pressing the
 *	play button causes playback to resume.
