...
void AssetDescriber::setupDescriptions()
{
	...
	curr->addSFunc("cacheFile(fileElementName)", "If an audio file is set to an Audio Element for the first time during the course of an experiment, the element must perform the overhead necessary to create a new underlying 'sound' and this could cause some lag in experimental execution.  The system caches the sound from the AudioFileParameter stored in the AudioFile field automatically, but it can't predict what other AudioFileParameter's sound it might need to cache.  To avoid this, call cacheFile(fileElementName) with each additional file that can be played back in this audio element at the beginning of the experiment so that the element can preload all sounds that it will need to play.");
	...
	curr->addProp("YXSignalShear", "A shear value used to shear the vertical eye input as a function of horizontal x input.  Hopefully, you won't need to use this often.");
	...
	curr->addSProp("yxSignalShear", "Sets the value of XYSignalShear.  See that property.");
	...
	curr->addSFunc("randomize()", "Generates the next pseudorandomly selected value and makes it accessible from the 'value' script property.");
	...
	curr->addProp("Max", "The maximum value in the range of values that will be randomized.");
	curr->addSProp("value", "Sets/Gets the current value stored in this element.");
	curr->addSFunc("randomize()", "Generates the next randomly selected value and makes it accessible from the 'value' script property.");
	...
	curr = addDescription("TargetFactory");
	curr->setInherits(getAssetDescription("ControlTarget"));
	curr->setOverview("The Target Factory is a set of individual 'targets' that have individual shapes, colors, sizes and positions that can be set during runtime by the designer.  This factory is particularly useful if you have a widely variable number of control targets that will need to be used during experimentation.  This factory is quite robust and allows for a whole lot of different types of configurations.  In general, the design properties are used to set default values for the targets and the various values of individual targets are then set and changed by scripts during the session run.  See the properties and functions for further details.");
	curr->addProp("TargetShape", "The shape (Ellipse,Rectangle) of the targets in the factory.");
	curr->addProp("TargetSize", "The size of the targets in the factory.");
	curr->addProp("NumTargets", "The number of targets in the factory.");
	curr->addSProp("targetWidth", "Sets/Gets default width of the targets.");
	curr->addSProp("targetHeight", "Sets/Gets default height of the targets.");
	curr->addSProp("targetShape", "Sets/Gets default shape of the targets ('Ellipse','Rectangle','Diamond').");
	curr->addSProp("numTargets", "Sets/Gets the number of targets in the factory.");
	curr->addSProp("fixatedIndex", "Gets the index of either the current target being fixated on, or the last target successfully fixated on. Returns -1 if no target has been fixated on yet, or if fixation was broken.");
	curr->addSFunc("setTargetDimensions(w,h)", "Sets the default width and height of this factory's targets in a single function call.");
	curr->addSProp("setTargetPos(index,x,y)", "Sets the position of the target at the input index in global coordinates (ie. top left corner of screen is 0,0).");
	curr->addSProp("setTargetLocalPos(index,x,y)", "Sets the position of the target at the input index in local coordinates (ie. Target at (0,0) has the same position as the target factory.  Target at (10,10) has position 10 pixels to the right of and 10 pixels below the position set for the target factory.");
	curr->addSProp("setTargetX(index,x)", "Sets the x component of the position of the target at the input index in global coordinates (ie. top left corner of screen is 0,0).");
	curr->addSProp("setTargetY(index,y)", "Sets the y component of the position of the target at the input index in global coordinates (ie. top left corner of screen is 0,0).");
	curr->addSProp("setTargetLocalX(index,x)", "Sets the x component of the position of the target at the input index in local coordinates (ie. Target at (0,0) has the same position as the target factory.  Target at (10,10) has position 10 pixels to the right of and 10 pixels below the position set for the target factory.");
	curr->addSProp("setTargetLocalY(index,y)", "Sets the y component of the position of the target at the input index in local coordinates (ie. Target at (0,0) has the same position as the target factory.  Target at (10,10) has position 10 pixels to the right of and 10 pixels below the position set for the target factory.");
	curr->addSFunc("setTargetSize(index,size)", "Scales the size of the target at the input index by a factor of the input size, where size is a number between 0 and 1. In case of Triangle targets, size (width and/or height) is twice the radius of the circle in which the triangle is inscribed.");
	curr->addSFunc("setTargetColor(index,r,g,b,a) or setTargetColor(index,r,g,b)", "Sets the color of the target at the input index according to the red, green, blue and optional alpha inputs.");
	curr->addSFunc("setTargetColor(index,color)", "Sets the color of the target at the input index by using a string containing hexadecimal r,g,b values (#rrggbb).");
	curr->addSFunc("setTargetShape(index,shape)", "Sets the shape of the target at the input index according to the input shape ('Ellipse','Rectangle').");
	curr->addSFunc("setTargetEnabled(index, enabled)", "Sets whether this target is enabled or not. A disabled target will not be drawn or allowed to be fixated on.");
	curr->addSFunc("getTargetX(index)", "Gets the x component of the position of the target at the input index in global coordinates (ie. top left corner of screen is 0,0).");
	curr->addSFunc("getTargetY(index)", "Gets the y component of the position of the target at the input index in global coordinates (ie. top left corner of screen is 0,0).");
	curr->addSFunc("getTargetLocalX(index)", "Gets the x component of the position of the target at the input index in local coordinates (ie. Target at (0,0) has the same position as the target factory.  Target at (10,10) has position 10 pixels to the right of and 10 pixels below the position set for the target factory.");
	curr->addSFunc("getTargetLocalY(index)", "Gets the y component of the position of the target at the input index in local coordinates (ie. Target at (0,0) has the same position as the target factory.  Target at (10,10) has position 10 pixels to the right of and 10 pixels below the position set for the target factory.");
	curr->addSFunc("getTargetSize(index)", "Gets the scale factor from 0 to 1.0 of the target at the input index.");
	curr->addSFunc("getTargetRed(index)", "Gets the red component of the color of the target at the input index.");
	curr->addSFunc("getTargetGreen(index)", "Gets the green component of the color of the target at the input index.");
	curr->addSFunc("getTargetBlue(index)", "Gets the blue component of the color of the target at the input index.");
	curr->addSFunc("getTargetAlpha(index)", "Gets the alpha component of the color of the target at the input index.");
	curr->addSFunc("getTargetColor(index)", "Gets the color value of the target at the input index as a string containing hexadecimal r,g,b values (#rrggbb).");
	curr->addSFunc("getTargetShape(index)", "Gets the shape of the target at the input index ('Ellipse','Rectangle').");
	curr->addSFunc("getTargetEnabled(index)", "Gets whether this target is enabled or not. A disabled target will not be drawn or allowed to be fixated on.");
