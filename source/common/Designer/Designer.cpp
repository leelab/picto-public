...
/*! \brief Called when the zoom spin box is updated to change the zoom level of the designer canvas.*/
void Designer::sceneScaleChanged(double scale)
{
	sceneScaleValue_ = (int)(scale * 100);
	sceneScaleSpin->setValue(sceneScaleValue_);

}

/*! \brief Called when the zoom spin box is updated to change the zoom level of the designer canvas.*/
void Designer::sceneScaleChanged()
{
	double newScale = sceneScaleSpin->value() / 100.0;
	editorState_->setZoom(newScale);
}
...
void Designer::loadScene(DiagramScene* newScene)
{
	...
	connect(view, SIGNAL(zoomChanged(double)), this, SLOT(sceneScaleChanged(double)));
}
...
void Designer::createToolbars()
{
	...
	sceneScaleSpin = new QSpinBox;
	sceneScaleSpin->setRange(30, 200);
	sceneScaleSpin->setSuffix("%");
	sceneScaleSpin->setValue(100);
	sceneScaleSpin->setSingleStep(10);
	connect(sceneScaleSpin, SIGNAL(valueChanged(int)),
	...
	pointerToolbar->addWidget(sceneScaleSpin);
	pointerToolbar->addSeparator();
