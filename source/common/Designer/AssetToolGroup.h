...
/*! \brief A ToolGroup with buttons representing Asset objects that can be added to the Designer as children of the
 *	current Window Asset.
 *	\details In practice, this is the only ToolGroup that we are using.  When a designer wants to add a new element to the
 *	current Window Asset, they click on that Asset's button, then click on the canvas and behind the scenes the Asset
 *	represented by the button is added as a child to the Window Asset and a corresponding DiagramItem representing that
 *	Asset is added to the canvas at the input position.
 *	Asset buttons are greyed out when the maximum number of that button's Asset have already been added to the current
 *	Window Asset.
