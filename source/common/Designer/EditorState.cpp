...
/*! \brief Returns a list of the current SearchRequest objects.  All searchable elements in the Designer should be serching for these.
 *	\sa SearchRequest
 */
 ...
 *	\details When there is an active Analysis, things are displayed differently on the
 *	Designer canvas.  AnalysisElements specific to the current Analysis are shown.  Experimental
 *	elements are grayed out.  Only AnalysisElements are shown in the Toolbox.
 *	Since Analyses can exist on their own and be attached to different experiments, this function
 *	first checks to see if the input Analysis is already linked to the Experiment being designed.
 *	If it is not linked to the Experiment, the function links it.  Any incompatibilities or other
 *	problems in the link are reported to the user in a message box.  Then the Analysis is set to
 *	active in DesignConfig::setActiveAnalysisIds() and the currentAnalysisChanged() signal is emitted.
 */
 ...
 *	\details The input zoom will be limited to the range (.3,2).  When called,
 *	this function emits zoomChanged() with the new zoom value.  The zoom value is 
 *	essentially a constant by which the size of each object will be multiplied.
 *	\note Don't confuse this zoom's functionality with the zoom used in the TestViewer and RemoteViewr
 *	windows to zoom in and out of a running experiment.  That zoom is defined differently
 *	from this one.
 */
 ...
 *	in the UI.  Category and type are the name of the XML tag and its type property value used to store this Asset within the 
 *	WindowAsset's XML tag.  pixmap is the Asset's icon which can replace the mouse icon to show the designer what type of 
 *	Asset they are about to create before they click.  This function emits the insertionItemChanged() function.
 *	\note When this function is called, the EditorState::EditMode is changed from EditorState::Select to EditorState::PlaceItem or the opposite
 *	depending if the input value is an insertable item or empty data (indicating that there is no current insertable item).
 */
 ...
 *	\details If undoable is true, an undo point will be set after changing the window
 ...
 *	\details Internally, this function checks the input item's type and calls setSelectedAsset()
 *	with an appropriate input.  If the input item is an AssetItem or a sub-graphic, it calls setSelectedAsset() with
 *	the referenced Asset.  If it is anything else, it calls setSelectedAsset() with the current Window Asset.
 *	This function emits the itemSelected() or arrowPortSelected() signals as appropriate.
 * \sa setSelectedAsset()
 */
 ...
 *	\details This function clears the insertion item, then emits the windowAssetChanged() signal with the current Window Asset
 *	and then emits selectedAssetChanged() with the current selected Asset.  This has the effect of telling all widgets connected
 *	to these signals to reload themselves.
 *	\note This function does not appear to be in use right now.  If you decide to use it, it is probably worth testing to make sure
 ...
 *	\details When called, the searchRequested() signal is emitted.  All searchable elements in the Designer should respond to 
 *	that signal by searching for all active searchRequests returned from getSearchRequests().
 *	\sa SearchRequest
 */
