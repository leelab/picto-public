...
/*! \brief This \#define should be copied into the public section of anything inheriting AssociateElement in order to
 *	implement its pure virtual functions.
 ...
 *	\details Current AssociateElements are used for Analysis and UI information in the Designer.  Neither of these need
 *	to be included for the experiment to run, but they do need to be addable into the Design to provided their additional
 *	functionality.  For this reason, they "link into" design elements.  They are deleted when design elements are deleted
 *	and their linked ids and paths are updated when the elements to which they are linked are edited; however, they can
 *	easily be unlinked from design elements without affecting the integrity of the design.
 *	\note In order to properly inherit from AssociateElement, descendant classes need to copy the
 *	ASSOCIATE_ELEMENT_IMPLEMENTATION \#define into the public section of their .h file and copy the
 *	EXP_LINK_FACTORY_CREATION \#define to the end of their constructor.  We don't just include this code in the
 *	AssociateElement itself because it would not complile unless the AssociateElement class inherited from DataStore.
 *	AssociateElement children need to inherit from various DataStore classes themselves to be able to reuse DataStore
 *	sub-class code, and this would therefore lead to AssociateElement children having diamond inheritance patterns
...
public:
	 ...
	 *	\details Adds a required child HostLink to the AssociateElement to store information about the Design element
	 *	to which the AssociateElement should be linked.
	 */
	AssociateElement(){};
	virtual ~AssociateElement(){};
	/*! \brief Returns the Path of the Design element to which this AssociateElement is linked (this is handled by a child
	 *	AssociateHostLink).
	 */
	virtual QString getParentPath() = 0;
	/*! \brief Returns the Asset ID of the Design element to which this AssociateElement is linked (this is handled by a
	 *	child AssociateHostLink).
	 */
	virtual int getParentId() = 0;
	/*! \brief Updates and connects linked asset data fields for the input Asset (this is handled by a child
	 *	AssociateHostLink).
	 */
	...
	/*! \brief Attempts to attach this AssociateElement as an Associate Child to the current linked asset
	 *	(set in linkToAsset()).
	 *	\details If tryLinkingById is true, we first attempt to link by looking up the Asset ID in the AssociateRootHost
	 *	to which our parent AssociateRoot is attached.  If that doesn't work, or the input was false, we attempt to
	 *	link by linked path.  Returned value is an AssociateElement::AttachmentResult.
	 */
	...
	/*! \brief Changes the section at the beginning of the linked path which is input as oldPrefix to the input newPrefix
	 *	value.
	 *	\details This is useful when exporting a group of AnalysisElements from one element and importing them into
	 *	another one.  In that case, we want to keep all of the imported element's relative paths the same but change their
	 *	parent path.
	 */
