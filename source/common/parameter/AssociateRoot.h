...
/*! \brief AssociateRoot objects are the root of a tree with elements that connect to an AssociateRootHost object's tree and provide additional information 
 *	about the AssociateRootHost tree that does not affect its basic functionality.
 *	\details By separating out the AssociateRoot from the AssociateRootHost, we can add or remove AssociateRoot objects without affecting the integrity of the 
 ...
 *	
 *	Like AssociateRootHosts, AssociateRoots also have an ID.  When AssociateElement children of an AssociateRoot are linked to elements in an AssociateRootHost's 
 *	tree, they are linked according to their AssociateRoot parent's ID.  This way, even if many different AssociateRoots are attached to a single
 *	AssociateRootHost, we can quickly enable and disable them by simply telling the AssoicateRootHost to pay attention to or ignore AssociateElements with
...
public:
	...
	/*! \brief Returns this object's Associate ID.  All AssociateElement children of this object are linked to an AssociateRootHost's descendants
	 *	according to this ID.
	 */
	QUuid getAssociateId(){return propertyContainer_->getPropertyValue("AssociateId").toUuid();};
	/*! \brief Returns the Host ID of the AssociateRootHost that this AssociateRoot is linked to.  If no
	 *	AssociateRootHost is linked, returns an invalid QUuid.
	 */
	QUuid getLinkedHostId(){return propertyContainer_->getPropertyValue("LinkedHostId").toUuid();};
	/*! \brief Returns the name of the AssociateRootHost that this AssociateRoot is linked to.  If no
	 *	AssociateRootHost is linked, returns an empty name.
	 */
	 ...
	 *	\note This should probably be renamed to getLinkedHost() or something like that.  It is named this
