 ...
 *	Stores a pointer to the input Asset.  Connects to the input Asset's Asset::assetIdEdited() and UIEnabled::nameEdited()
 *	signals so that this object will know when it has to change the values stored in its Properties, then calls
 *	updateLinkedAssetProperties() to update those Property values for this new Asset and emits the linkedToAsset() signal
 *	to tell the world that the linked Asset changed.
 */
void AssociateHostLink::linkToAsset(QSharedPointer<Asset> asset)
{
	...
	{
		connect(linkedAsset_.data(),SIGNAL(assetIdEdited()),
			this,SLOT(updateLinkedAssetProperties()));
		connect(linkedAsset_.toStrongRef().staticCast<UIEnabled>().data(),
			SIGNAL(nameEdited()),this,SLOT(updateLinkedAssetProperties()));		
	}
	...
	connect(linkedAsset_.data(),SIGNAL(assetIdEdited()),
		this,SLOT(updateLinkedAssetProperties()));
	connect(linkedAsset_.toStrongRef().staticCast<UIEnabled>().data(),SIGNAL(nameEdited()),
		this,SLOT(updateLinkedAssetProperties()));
...
/*! \brief Updates the current linked path value by searching for the input oldPrefix string at the beginning of the path
 *	and replacing it with the newPrefix string.
 *	\details This is useful when we are exporting and importing multiple AssociateElements from one element into another.
 *	In those cases, we want to maintain relative link paths for the different AssociateElements but change the overall
 *	location of the group.  This is equivalent to updating the beginning of all of their paths in the same way.
 */
