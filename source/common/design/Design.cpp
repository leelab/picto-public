...
//	QSharedPointer<DataStore> newRoot = createRoot(root_->identifier());
//	while(!xmlReader->isStartElement() && (xmlReader->name() != newRoot->identifier())) 
//		xmlReader->readNext();
//	if(xmlReader->atEnd())
//	{
//		return "Could not find " + newRoot->identifier() + " in Design Text.";
//	}
//
