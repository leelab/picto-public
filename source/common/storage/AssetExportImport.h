...
/*! \brief Provides functionality for exporting selected assets and importing them into a separate experiment or the same experiment at a different location.
 *	\details This class is used for copy/paste functionality and larger scale importing of entire Analyses into new experiments during the design process
 *	or to Analyze a session that didn't have an appropriate Analysis saved with the design when the session was run.
 *	With this class, Experimental and Analysis elements must be exported separately.  Since Analysis elements don't define a tree structure but are added into
 *	an experimental tree structure, import of analysis elements can be partially succesful where some Analyses find their correct parent elements and some don't.
