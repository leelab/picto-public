...
{
	//Since receiveEditedSignal and receiveDeleteSignal are the places where the edited_ and
	//deleted_ flags are actually set, we connect our edited() and deleted() signals to them.
 ...
 *	\details If the asset has been marked deleted, its returned XML code is empty.  Also,
 *	preSerialize() is called before the actual serialization begins and a boolean is 
 *	returned indicating whether the Asset was successfully serialized.
 */
 ...
 *	\details Before deserialization starts, the preDeserialize() function is called.
 *	After serialization, edited_, isNew_ and deleted_ flags are all set to false, and
 *	if deserialization was successful, postDeserialize() is called and true returned.  
 *	If deserialization was not successful, false is returned and postDeserialize is 
 *	not called.
 */
 ...
 *	\details It is awkward to call validateTree on an Asset which has no way to 
 *	store children.  Probably, this function should be in the DataStore class and
 *	Asset should only expose validateObject().  It might be a good idea to change
 *	that at some point, but be very careful.  This all results from the fact Asset
 *	object's can all have parents but can't all have kids.
 */
 ...
 *	\details Assets are not ready to be used when they are first created.  Much of their initialization
 *	occurs in the postDeserialize() function that is only called when serialization is complete. 
 *	They therefore need to have some XML deserialized even if it is just an empty tag in order to be 
 *	sure that they are initialized and ready to be used.  This function essentially creates an empty 
 *	tag using the passed in tagName and type and deserializes this asset from that XML. When deserialization 
 *	from the empty tag is complete, the Asset is marked asNew_ since no actual saved XML data had to be used 
 *	in setting it up.  In some cases, the fact that the Asset is marked asNew_ will mean that it does
...
/*! \brief Initializes the asset from the input XML without marking it as "not new" as would generally occurs as soon was deserialized from XML.  
 *	\details Essentially, this function just calls fromXml() and then sets isNew_ to true.  This way the serialization system
 *	can recognize that this asset was initialized to some value but wasn't actually serialized in from scratch.  This is useful
 *	in some of the complex Serialization logic, particularly in DataStore.
 *	\sa DataStore::deserializeFromXml(), DataStore::serializeAsXml()
 */
...
/*! \brief Emits the deleted() signal when called to inform interested parties that this asset needs to be removed from the design.
 ...
 *	\note Experiment=TestExp, Task=ATask, TopStateMachine=Top, Element=FirstState 
 *	would have path: "ATask::Top::FirstState"
 */
 ...
 *	\attention Classes that extend this function must always start the function by calling 
 *	their parent class's implementation of upgradeVersion().
 */
 ...
 *	\details Every Design has a single DesignConfig object that manages global issues 
 *	affecting the entire design.  It can access every asset in the design and is 
 *	accessable from every asset in the design.  It is set to an asset using the
 *	setDesignConfig function, typically by the Asset's parent.
 *	\note There is high potential here for a memory leak due to circular shared
 *	pointers.  To avoid this, Assets maintain SharedPointers to the DesignConfig
 *	but the DesignConfig uses WeakPointers to all assets.
...
/*! \brief This function is called in fromXml() before deserialization and is used to prepare the Asset for deserialization.
 *	\details In the case of the Asset class, during preDeserialize we get the object's parent object's
 *	DesignConfig and set it to this Asset.
 *	\attention Any descendant classes extending this function should begin the function by calling their
 *	parent's implementation of preDeserialize().
 */
...
/*! \brief This function is called in fromXml() after successful deserialization and is used to initialize the Asset based on values that were loaded in during deserialization.
 *	\details In the case of the Asset class, during postDeserialize we tell the DesignConfig that if we have reached the stage of loading
 *	where Asset IDs must be unique, it should make sure that our Asset ID is different from all others in this design.
 ...
 *	\details This function takes the input error message, adds the path to this asset, formats a
 *	new 'pretty' error message accordingly, and calls Serializable::addErrorToList() with the formatted
 *	message.
