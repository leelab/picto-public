...
namespace Picto {
/*! \brief A base class for all Picto data types that need to be individually identifiable and communicatable over the
 *	network.
 *	\details This class extends the Serializable class to add Data ID functionality, such that descendants of this class
 *	can be serialized over the network, and when they do so will automatically include a Data ID that is unique and
 *	ascending from one resetDataIDGenerator() call to the next.
 *	\note In Picto, resetDataIdGenerator() is called by the Director every time a new Session is started such that Data ID
 *	values are unique over a single session.  Since many behavioral data values are marked by the frame in which they
 *	occured, the Data ID is useful to determine the order in which Behavioral Data Values were generated.  The uniqueness of
 *	the ID also lets us assure that data values that might be sent more than once due to network problems are still only
 *	stored once in the session database.
...
public:
	 ...
	 *	\details The output XML is meant to be used with a deserializeFromXml() function on the same type of DataUnit.
	 */
	 ...
	 *	\details Typically the XML of the input QXmlStreamReader will have been created using serializeAsXml()
	 */
