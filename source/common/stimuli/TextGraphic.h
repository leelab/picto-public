	...
	*/
	class PICTOLIB_CLASS TextGraphic : public VisualElement
	{
		Q_OBJECT
			...
			/*! \brief Sets/gets the default font size.*/
			Q_PROPERTY(int FontSize READ getSize WRITE setSize)
	...
	public:
		...
		*/
		...
		int getSize() { return propertyContainer_->getPropertyValue("FontSize").toInt(); };
		/*! \brief Sets the text that is displayed in the graphic.*/
		void setSize(int fontsize) { propertyContainer_->setPropertyValue("FontSize", fontsize); };
		...
		void setFontSize(int s){ setSize(s); };
