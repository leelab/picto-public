...
void MainWindow::changeMode(Viewer* nextViewer)
{
	...
	//Update the recent files list with viewer info
	if (!recentFiles_.isEmpty())
		recentFiles_.removeFirst();
	recentFiles_.prepend(currFile_ + ";" + nextViewer_->type());
...
*	in the file dropdown is updated along with its correspoding recent viewer and the window title.  Then changeMode() is called with the current Viewer
...
void MainWindow::setCurrentFile(const QString &filename)
{
	...
	if (!currFile_.isEmpty())
	{
		...
		foreach(QString foundFile, recentFiles_)
		{
			QString fileName = QFileInfo(foundFile.section(';', 0, 0)).fileName();
			if (fileName == shownName)
			{
				//retrieve corresponding viewer and remove file from list
				QString viewerName = tr("Untitled");
				viewerName = foundFile.section(';', 1, 1);
				if (viewerName == "State Edit Viewer")
					currentIndex = 0;
				else if (viewerName == "Test")
					currentIndex = 1;
				else if (viewerName == "Remote")
					currentIndex = 2;
				else if (viewerName == "Replay")
					currentIndex = 3;

				recentFiles_.removeAll(foundFile);
				break;
			}
		}
	...
	if (currentIndex > -1)
	{
		//recent files list is updated with viewer info in changeMode
		viewerStack_->setCurrentIndex(currentIndex);
		changeMode(qobject_cast<Viewer*>(viewerStack_->currentWidget()));
	}
	else
	{
...
void MainWindow::updateRecentFileActions()
{
	...
	foreach(QString foundFile, recentFiles_)
	{
		QString filename = foundFile.section(';', 0, 0);
