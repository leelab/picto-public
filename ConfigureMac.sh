...
#                        mark it as executable so that the Mac shell will run it.
...
#We don't export DYLD_LIBRARY_PATH as we set embed our libraries in the app bundle as part of the build process
#export DYLD_LIBRARY_PATH=$PICTO_TREE/intermediates/lib/debug:$PICTO_TREE/intermediates/lib/release
...
echo "  i.e. \". ./Configure_Mac.sh\" so that the environment is"
echo "  propagated to the active terminal."
echo "Be sure the build environment for your compiler and Qt is also prepared"
echo "Use the GenerateBuildFiles.sh script in tools/mac to build"
