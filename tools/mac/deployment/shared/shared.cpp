...
void deployQtFrameworks(const QString &appBundlePath)
{
   ...
   QStringList bundledLibs;
   QStringList bundledLibDirs = QDir(appBundlePath + "/Contents/Libraries").entryList();
   foreach (QString bundledLibDir, bundledLibDirs)
   {
      QStringList libFiles = QDir(appBundlePath + "/Contents/Libraries/" + bundledLibDir).entryList(QStringList() << "*.dylib");
      foreach (QString libFile, libFiles)
      {
         bundledLibs += bundledLibDir + "/" + libFile;
      }
   }
   foreach (QString bundledLib, bundledLibs)
   {
      applicationBundle.binaryPath = appBundlePath + "/Contents/Libraries/" + bundledLib;
      deployQtFrameworks(getQtFrameworks(applicationBundle.binaryPath), applicationBundle.path, applicationBundle.binaryPath);
   }
}
